        ...
        if ($_POST['action'] == "add") {
            ...
            if ($total_bookings > 0) {
                ...
                if ($total_bookings_same_time >= 3) {
                    ...
                    $sql = "INSERT INTO bookings (booking_date, booking_time, booking_name) VALUES (?, ?, ?)";
                    $stmt = $pdo->prepare($sql);
                    if ($stmt->execute([$booking_date, $booking_time, $booking_name])) {
        ...
        <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody>
                        <?php
                        $sql = "SELECT id, booking_date, booking_time, booking_name FROM bookings ORDER BY booking_date, booking_time";
                        $stmt = $pdo->query($sql);
                        if ($stmt->rowCount() > 0) {
                            ...
                            echo "<tr><td colspan='4' class='py-4 px-6 border-b border-grey-light text-center'>Tidak ada booking tersedia.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
